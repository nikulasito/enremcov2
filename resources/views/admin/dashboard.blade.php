<x-admin-v2-layout title="ENREMCO Admin Dashboard | Main Overview" pageTitle="Dashboard" pageSubtitle="System Overview">
    @php
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-10">
        ...
        <div class="summary-card">
            <div class="flex items-center justify-between mb-5">
                ...
                <div
                    class="flex items-center gap-1 text-[11px] font-bold text-slate-500 bg-slate-50 px-2 py-0.5 rounded-full">
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-slate-50/50 border-b border-slate-100">
                            <th class="px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">
                                Applicant Name</th>
                            <th class="px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">Office
                            </th>
                            <th class="px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">Date
                            </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="size-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 text-xs font-bold">
                                            {{ $initialsOf($m->name ?? '') }}
                                        </div>
                        ...
                        @empty
                            <tr>
                                <td class="px-6 py-6 text-sm text-slate-500" colspan="3">No pending membership requests
                                    found.</td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                ...
                <a href="{{ route('admin.loan-requests.index') }}"
                    class="p-2 hover:bg-slate-50 rounded-lg text-primary">
                    <span class="material-symbols-outlined">open_in_new</span>
                </a>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-slate-50/50 border-b border-slate-100">
                            <th class="px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">Borrower
                            </th>
                            <th class="px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">Loan
                                Type</th>
                            <th class="px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">Amount
                            </th>
                            <th class="px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">Status
                            </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @forelse(($recentLoanEntries ?? collect()) as $loan)
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                @php
                                    $borrower = $loan->full_name ?? optional($loan->user)->name ?? '—';
                                @endphp
                                <td class="px-6 py-4 text-sm font-extrabold text-slate-900">{{ $borrower }}</td>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span
                                            class="status-pill bg-amber-50 text-amber-600 border border-amber-100">Pending</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
